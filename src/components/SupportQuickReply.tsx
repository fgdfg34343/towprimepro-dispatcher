...
interface SupportQuickReplyProps {
...
const SupportQuickReply = ({ chat, onClose, onOpenFull }: SupportQuickReplyProps) => {
  ...
  useEffect(() => {
    ...
    const unreadMessages = messages.filter(
      (message) => message.senderType === "driver" && !message.isRead,
    );
    ...
    const markAsRead = async () => {
      try {
        ...
        unreadMessages.forEach((message) => {
          const messageRef = doc(db, "supportChats", chatId, "messages", message.id);
          batch.update(messageRef, { isRead: true });
        });

        await batch.commit();
      } catch (error) {
        console.error("[SupportQuickReply] Не удалось отметить сообщения как прочитанные:", error);
      }
    };
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      console.error("[SupportQuickReply] Ошибка отправки сообщения:", error);
    ...
    <div className="fixed bottom-6 right-6 z-50 w-full max-w-sm animate-in fade-in slide-in-from-bottom-2">
      <Card className="flex h-[420px] flex-col border border-border/80 bg-background shadow-2xl shadow-black/30">
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          <div>
            <p className="text-sm font-semibold text-foreground">{chat.driverName}</p>
            <p className="text-xs text-muted-foreground">{chat.driverPhone || "Нет телефона"}</p>
          </div>
...
export default SupportQuickReply;
